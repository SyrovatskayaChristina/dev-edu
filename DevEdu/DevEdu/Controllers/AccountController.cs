...
namespace DevEdu.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            var user = await userStorage.UserGetByLogin(currentUser.Login);
            var identity = await GetIdentity(user, currentUser);
            if (identity == null)
            {
        ...
        private async Task<ClaimsIdentity> GetIdentity(User user, UserLoginPassInputModel currentUser)
        {
            //ищем пользователя по логину

            // сравниваем введенный пароль с хешем пароля в базе
            if (user == null || !Hashing.ValidateUserPassword(currentUser.Password, user.Password)) return null;
