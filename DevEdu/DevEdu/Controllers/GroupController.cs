...
namespace DevEdu.Controllers
{
    ...
    public class GroupController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("groups-by-teacher-with-course-program/{userId}")]
        public async Task<ActionResult<IEnumerable<GroupWithCourseProgramOutputModel>>> GetAllGroupWithCourseProgramByUser(int userId)
        {
            if (userId.Equals(null)) return BadRequest("Group Id is empty");
            var result = GroupMapper.ToGroupsWithCourseProgram(await groupStorage.GetTeacherGroupsWithCourseProgramById(userId));
            if (result.Equals(null)) return BadRequest("Failed to add object");
            return Ok(result);
        }
